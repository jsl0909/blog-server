...
var Node = class _Node {
  ...
  */
  static fromJSON(schema2, json) {
    ...
    let content3 = Fragment.fromJSON(schema2, json.content);
    let node2 = schema2.nodeType(json.type).create(json.attrs, content3, marks);
...
function nfa(expr) {
  ...
  function compile2(expr2, from) {
    if (expr2.type == "choice") {
      ...
      for (let i = 0; ; i++) {
        let next = compile2(expr2.exprs[i], from);
...
function gatherMarks(schema2, marks) {
  let found2 = [];
  for (let i = 0; i < marks.length; i++) {
    let name = marks[i], mark = schema2.marks[name], ok3 = mark;
    if (mark) {
      found2.push(mark);
    } else {
  ...
  constructor(schema2, rules) {
    ...
    this.normalizeLists = !this.tags.some((r) => {
      if (!/^(ul|ol)\b/.test(r.tag) || !r.node)
        return false;
      let node2 = schema2.nodes[r.node];
...
_ParserState.create = (schema2, remark) => {
  const state = new _ParserState(schema2);
  return (text4) => {
...
_SerializerState.create = (schema2, remark) => {
  const state = new _SerializerState(schema2);
  return (content3) => {
    state.run(content3);
  ...
  */
  insertText(text4, from, to) {
    ...
    if (from == null) {
      ...
      return this.replaceSelectionWith(schema2.text(text4), true);
    } else {
      if (to == null)
        to = from;
      to = to == null ? from : to;
      ...
      this.replaceRangeWith(from, to, schema2.text(text4, marks));
  ...
  return (state, dispatch) => {
    ...
    for (let d = $from.depth; ; d--) {
      ...
      if (node2.isBlock) {
        ...
        types2.unshift(splitType || (atEnd && deflt ? { type: deflt } : null));
        splitDepth = d;
        break;
      } else {
        if (d == 1)
...
function isTextOnlySlice(slice) {
  if (slice.content.childCount === 1) {
    const node2 = slice.content.firstChild;
...
function flatten(node2, descend = true) {
  const result = [];
...
function findChildren(predicate) {
  return (node2, descend) => flatten(node2, descend).filter((child) => predicate(child.node));
}
...
function getNodeFromSchema(type, schema2) {
  const target = schema2.nodes[type];
  ...
  if (asText) {
    ...
    if (parsed) {
      ...
      text4.split(/(?:\r\n?|\n)+/).forEach((block) => {
        ...
        if (block)
          p.appendChild(serializer2.serializeNode(schema2.text(block, marks)));
      });
    }
  } else {
...
function readDOMChange(view, from, to, typeOver, addedNodes) {
  ...
  if (ie3 && ie_version2 <= 11 && change.endB == change.start + 1 && change.endA == change.start && change.start > parse2.from && parse2.doc.textBetween(change.start - parse2.from - 1, change.start - parse2.from + 1) == "  ") {
    change.start--;
    change.endA--;
    change.endB--;
  }
...
var editorState = (ctx) => {
  ...
  return async () => {
    ...
    const doc4 = getDoc(defaultValue, parser2, schema2);
...
var EditorStatus = ((EditorStatus2) => {
...
//# sourceMappingURL=chunk-RMA7BMCH.js.map
