...
// ../node_modules/@milkdown/plugin-prism/lib/index.js
function flatNodes(nodes, className = []) {
  return nodes.flatMap(
    (node) => {
      var _a;
      return node.type === "element" ? flatNodes(node.children, [
        ...className,
        ...((_a = node.properties) == null ? void 0 : _a.className) || []
      ]) : [{ text: node.value, className }];
    }
  );
}
function getDecorations(doc, name, refractor2) {
  const { highlight: highlight2, listLanguages: listLanguages2 } = refractor2;
  const allLanguages = listLanguages2();
  const decorations = [];
  findChildren((node) => node.type.name === name)(doc).forEach((block) => {
    ...
    const nodes = highlight2(block.node.textContent, language);
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply: (transaction, decorationSet, oldState, state) => {
        ...
        const codeBlockChanged = transaction.docChanged && (isNodeName || isPreviousNodeName || oldNode.length !== newNode.length || ((_a = oldNode[0]) == null ? void 0 : _a.node.attrs.language) !== ((_b = newNode[0]) == null ? void 0 : _b.node.attrs.language) || transaction.steps.some((step) => {
          const s2 = step;
          return s2.from !== void 0 && s2.to !== void 0 && oldNode.some((node) => {
            return node.pos >= s2.from && node.pos + node.node.nodeSize <= s2.to;
          });
        }));
        if (codeBlockChanged)
...
//# sourceMappingURL=@milkdown_plugin-prism.js.map
